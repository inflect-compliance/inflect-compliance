 ...
 * reconcile each affected user's `TenantMembership` through
 * `applyEntraGroupMapping` (EI-2) for near-real-time (de)provisioning —
 * complementing the pull-based claim path.
 *
...
import { applyEntraGroupMapping } from '../services/entra-group-mapper';
...
export async function scimPatchGroup(
    ...
    await runInTenantContext(ctxOf(ctx), async (db) => {
        ...
        // keep the linked EntraGroupMapping's display name in sync
        await db.entraGroupMapping.updateMany({
...
export async function scimDeleteGroup(ctx: ScimContext, id: string) {
    return runInTenantContext(ctxOf(ctx), async (db) => {
        const group = await db.scimGroup.findFirst({ where: { id, tenantId: ctx.tenantId } });
        if (!group) return { ok: false };
        await db.scimGroup.deleteMany({ where: { id, tenantId: ctx.tenantId } });
        // soft-deactivate the role mapping (deleting the group stops provisioning)
        await db.entraGroupMapping.updateMany({
            where: { tenantId: ctx.tenantId, aadGroupId: group.externalId },
            data: { isActive: false },
        });
 ...
 * then reconcile their role. Runs outside the group mutation's tenant context
 * so applyEntraGroupMapping owns its own.
 */
async function reconcileUsers(tenantId: string, userIds: string[]): Promise<void> {
    ...
    // applyEntraGroupMapping call that follows; batching would cross-contaminate.
    for (const userId of userIds) { // guardrail-allow: n+1
        ...
        await applyEntraGroupMapping(userId, tenantId, aadGroups, 'scim');
    }
}
